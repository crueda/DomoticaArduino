...
uint16_t print_webpage(uint8_t *buf)
{
  ...
  plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<?xml version='1.0'?><domotica><temperatura><estancia>estudio</estancia><valor>"));
  itoa(tempC,vstr,10);
  plen=es.ES_fill_tcp_data(buf,plen,vstr);
  plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("</valor></temperatura><temperatura><estancia>estudio (min)</estancia><valor>"));
  itoa(tempCmin,vstr,10);
  plen=es.ES_fill_tcp_data(buf,plen,vstr);
  plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("</valor></temperatura><temperatura><estancia>estudio (max)</estancia><valor>"));
...
uint16_t print_redirectpage(uint8_t *buf)
{
  ...
  plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<script> parent.location.href = 'http://www.iagodiaz.com/domotica/crueda'; </script>"));
  //cambiar
...
void loop(){
  ...
  while(1){
    ...
    if(++contador == 65535) {
        ...
        tempC = tempC + 4; // Corregido defecto de calibración -> le sumo 4 grados
